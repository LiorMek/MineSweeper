...
public class testing4 {
    public static void main(String[] args) {
        ...
        while (!gameOver) {
            ...
            } else if (coordinate.length == 3 && coordinate[0].equalsIgnoreCase("flag")) {
    ...
    public static boolean isValidCoordinate(int x, int y, int row, int col) {
        ...
        public static int[][] click(int x, int y, int[][] seen, int[][] grid) {
            ...
            // Check if the tile has already been revealed or flagged
            if (seen[x][y] != 0) {
                return seen;
            }
        
            // If the tile contains a mine, set it as revealed and return
            if (grid[x][y] == -1) {
                seen[x][y] = grid[x][y];
                return seen;
            }
        
            // Otherwise, mark the tile as checked (-7) and recursively explore adjacent tiles
            seen[x][y] = -7;
            ...
            for (int[] dir : directions) {
                ...
                seen = click(newX, newY, seen, grid);
            }
